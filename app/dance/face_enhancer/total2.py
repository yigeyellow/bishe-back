def total2(frame_count):
    print('2-3 my_enhance.py')
    print('修改了enhance中的dataset(改变了读取图片的路径)')
    print('修改了model加载路径:../checkpoints/face/40000_generator.pth')
    print('最终结果存在../data/face/full_fake/中')
    import model
    import dataset_enhance
    import cv2
    ...
    pose_name = './data/source/pose_source_norm.npy' # coordinate save every heads # 对应normalization.py里的
    ckpt_dir = './checkpoints/face'
    result_dir = './results'
    save_dir = dataset_dir+'/full_fake/' # '../data/face/full_fake/' 
    ...
    image_folder = dataset_enhance.ImageFolderDataset(dataset_dir, cache=os.path.join(dataset_dir, 'fake_local.db'), is_test=True)
    face_dataset = dataset_enhance.FaceCropDataset(image_folder, pose_name, image_transforms, crop_size=48)
    length = len(face_dataset)
    print('Picture number',length)
    print(len(image_folder))

    generator = load_models(os.path.join(ckpt_dir)) # ckpt_dir = '../checkpoints/face'
    ...
    for i in tqdm(range(frame_count)):
        ...
        cv2.imwrite(save_dir+ '{:05}.png'.format(i),fake_full) # '../data/face/full_fake/' 
